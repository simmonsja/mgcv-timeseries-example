...
$$flow = \beta_0 + \beta_t \cdot time + \epsilon \qquad \epsilon \sim N(0, \sigma^2)$$ 
...
draw(m3)
draw(m3, select="s(time)") # and specifically plot a single smooth term
...
# gratia has a function derivatives() so that we can see how the smooth is changing and how certain we are that it is increasing or decreasing
wrap_plots(
    pred_plot + scale_y_log10(),
...
-   This excellent article mostly focused on hierarchical methods but also with very comprehensive descriptions of `?mgcv`: Pedersen EJ, Miller DL, Simpson GL, Ross N. 2019. Hierarchical generalized additive models in ecology: an introduction with mgcv. PeerJ 7:e6876 [https://doi.org/10.7717/peerj.6876](https://doi.org/10.7717/peerj.6876)
